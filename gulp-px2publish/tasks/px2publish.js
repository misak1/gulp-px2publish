...
(function() {
  var M, _, _expand, _path, _rename_f, config, exec, fs, gulp, handleErrors, path, paths, px2publish, util;
  ...
  gulp.task("px2publish", function() {
    var fnCheck, fnCreateParam, target;
    target = void 0;
    path = M.require('path');
    ...
    if (target !== null && !/^\/px-files*/gm.test(target) && !/^\/caches*/gm.test(target)) {
      ...
      fnCheck = function(err) {
        ...
        if (!err) {
          ...
          return setTimeout(fnPublish, 3000);
        }
      };
      return fs.access(path, fnCheck);
    }
  });
